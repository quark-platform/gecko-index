...
export const guessTypeDefName = (esmName) => esmName.replace('.mjs', '.d.mts')

export const hasTypeDef = (esmName) =>
  existsSync(join(packageDir, 'data', 'esm', guessTypeDefName(esmName)))

export const getTypeDef = async (esmName) =>
  await readFile(
    resolve(packageDir, 'data', 'esm', guessTypeDefName(esmName)),
    'utf8'
  )

export const getExports = async () =>
  JSON.parse(
    await readFile(resolve(packageDir, 'data', 'esm', '_exports.json'), 'utf8')
  )
